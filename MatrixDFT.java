...
public class MatrixDFT {

	static final int nO_samples = 8; // number of samples in the input window
	static final int sample_rate = 30; // the sample rate of the input window in Hz
	
	static int loopNumber = 10000000;
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < loopNumber; ++i) {
			...
			for (int k = 0; k < nO_samples / 2; ++k) {
				for (int s = 0; s < nO_samples; ++s) {
					real_accumulator += final_input_y[s] * Math.cos(2 * Math.PI * k * s / nO_samples);
					complex_accumulator += final_input_y[s] * Math.sin(2 * Math.PI * k * s / nO_samples);
				}
